...
#include "HandsController.hpp"
#include "ShotEffector.hpp"
#include "HealthController.hpp"
#include "BreathEffector.hpp"
#include "WalkEffector.hpp"
#include "ProceduralWeaponAnimation.hpp"
#include "DizSkinningSkeleton.hpp"
#include "MovementContext.hpp"
#include "PlayerBody.hpp"
#include "StaminaParameters.hpp"
#include "Physical.hpp"
#include "Player.hpp"

#include "ObservedPlayerController.hpp"
#include "ObservedPlayerView.hpp"

#include "GameWorld.hpp"


#define _SILENCE_CXX17__CODECVT_HEADER_DEPRECATION_WARNING
...
struct mono_method_t {
	std::string name() {
		auto name = read_widechar(Driver->Read<uintptr_t>(reinterpret_cast<uintptr_t>(this) + 0x18), 128);
...
struct mono_vtable_t {
	OFFSET(flags(), byte, 0x30)
		...
		if ((this->flags() & 4) != 0)
			return Driver->Read<uintptr_t>(reinterpret_cast<uintptr_t>(this) + 0x40 + 8 * Driver->Read<int>(Driver->Read<uintptr_t>(reinterpret_cast<uintptr_t>(this) + 0x0) + 0x5c));
...
struct mono_class_t {
	...
	int get_num_methods() {
		const auto v2 = (Driver->Read<int>(reinterpret_cast<uintptr_t>(this) + 0x2a) & 7) - 1;
		switch (v2) {
		case 0:
		case 1:
			return Driver->Read<int>(reinterpret_cast<uintptr_t>(this) + 0xFC);
		...
		case 4u:
			return Driver->Read<int>(reinterpret_cast<uintptr_t>(this) + 0xF0);
...
namespace mono {
	inline mono_root_domain_t* get_root_domain() {
		return reinterpret_cast<mono_root_domain_t*>(Driver->Read<uintptr_t>(Storage->MonoDll + 0x499c78));
