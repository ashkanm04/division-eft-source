...
void c_config::load(size_t id) noexcept {
	...
	ArchiveX<std::ifstream>{ in } >> color;
	ArchiveX<std::ifstream>{ in } >> radar;
	ArchiveX<std::ifstream>{ in } >> flyhacksettings;
	in.close();
}
...
void c_config::save(size_t id) const noexcept {
	...
	ArchiveX<std::ofstream>{ out } << color;
	ArchiveX<std::ofstream>{ out } << radar;
	ArchiveX<std::ofstream>{ out } << flyhacksettings;
	out.close();
}
